...
@foreach ($dividers as $d)
    ...
    @foreach ($parentMenu as $m)
        ...
        <li class="nav-item" id="navigate_{{ $m['menu_id'] }}">
            <a href="{{ $route_parent }}" class="nav-link {{ (Request::segment(1) === $m['menu_route_name']) ? "active" : "" }}">
                ...
                @foreach ($childMenu1 as $c)
                    @php
                        if($c['menu_controller'] != '#'){
                            $route_child1 = route($c['menu_route_name'].'.index');
                        }else{
                            $route_child1 = '/#';
                        }
                    ...
                    <li class="nav-item" id="navigate2_{{ $c['menu_id'] }}">
                        @php
                            if(Request::segment(1) == $c['menu_route_name']){
                                $active1 = 'active';
                            }else{
                                $active1 = '';
                            }
                                ...
                                @foreach ($childMenu2 as $c2)
                                    @php
                                        if($c2['menu_controller'] != '#'){
                                            $route_child2 = route($c2['menu_route_name'].'.index');
                                        }else{
                                            $route_child2 = '/#';
                                        }
                                    @endphp
